        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 space-y-3">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 rounded-xl bg-emerald-50/70 border border-emerald-200/80 text-xs gap-2">
                        <div class="flex items-center gap-2">
                            ...
                            <span id="coord-display" class="text-emerald-800 font-medium">
                                {{ $product->location ?? 'Pin lokasi lahan terpasang di peta' }}
                            </span>
                        </div>
                    ...
                    <input type="hidden" id="latitude" name="latitude" value="{{ old('latitude', $product->latitude ?? '-7.8712') }}">
                    <input type="hidden" id="longitude" name="longitude" value="{{ old('longitude', $product->longitude ?? '112.5273') }}">
                </div>
...
<script>
    // Leaflet Interactive Map Picker for Farmer Edit Product
    let map, marker;
    ...
    window.detectGPSLocation = function() {
        ...
        const coordDisplay = document.getElementById('coord-display');
        if (coordDisplay) {
            coordDisplay.innerHTML = '<span class="text-amber-700 animate-pulse font-semibold">Mencari sinyal GPS...</span>';
        }
